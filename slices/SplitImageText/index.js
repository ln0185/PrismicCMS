 ...
 * @type {import("react").FC<SplitImageTextProps>}
 */
...
const SplitImageText = ({ slice }) => {
  return (
    ...
    >
      <div
        className={`${slice.variation === "heroSection" ? "pt-12 pb-24" : ""} flex justify-center items-center w-screen ${slice.primary.background_color ? backgroundColor[slice.primary.background_color] : backgroundColor.Grey}`}
      >
          ...
          <div className="flex flex-col gap-4 items-start">
            <PrismicNextImage
              ...
              className={`w-full h-auto rounded-3xl ${
                ["default", "splitButtonRight"].includes(slice.variation)
                  ? ""
                  : "md:order-last"
              }`.trim()}
            />
            <div className="flex flex-col gap-4 items-start">
              ...
              {["splitButtonLeft", "splitButtonRight", "heroSection"].includes(
                slice.variation
              ) && (
                <Button
